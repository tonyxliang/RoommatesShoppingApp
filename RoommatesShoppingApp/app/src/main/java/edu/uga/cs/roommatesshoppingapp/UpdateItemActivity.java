...
/**
 * This activity lets users update items in the shopping list.
 * They can also mark an item as purchased, or they can delete it from the list.
...
public class UpdateItemActivity extends AppCompatActivity {
    ...
    public static final String DEBUG_TAG = "UpdateItemActivity";
...
//        findUserAddress();
        ...
        markAsPurchased = findViewById(R.id.markAsPurchased);
        ...
        markAsPurchased.setOnClickListener(new MarkAsPurchasedButtonClickListener());
    }
    ...
    private class ButtonClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference(userAddress).child("Shopping List").child("Items").child(oldItemName);
...
*//*                    // Clear the EditTexts for next use.
    ...
    private class MarkAsPurchasedButtonClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            FirebaseDatabase database = FirebaseDatabase.getInstance();
            DatabaseReference myRef = database.getReference(userAddress).child("Purchased List");
            ...
            myRef.child("Items").child(newItemName).setValue(item).addOnSuccessListener(new OnSuccessListener<Void>() {
                ...
                public void onSuccess(Void aVoid) {
                    // Show a quick confirmation
                    Toast.makeText(getApplicationContext(), "Item marked as purchased",
                            Toast.LENGTH_SHORT).show();

                    DatabaseReference ref = FirebaseDatabase.getInstance().getReference(userAddress).child("Shopping List").child("Items").child(oldItemName);
                    ref.removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                        public void onSuccess(Void aVoid) {
                            Toast.makeText(getApplicationContext(), "Item removed from shopping list",
                                    Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(Exception e) {
                            Toast.makeText(getApplicationContext(), "Failed to remove item from shopping list",
                                    Toast.LENGTH_SHORT).show();
                        }
                    });
                ...
                @Override
                public void onFailure(Exception e) {
                    Toast.makeText(getApplicationContext(), "Failed to mark item as purchased",
                            Toast.LENGTH_SHORT).show();
                }
            });
        }
